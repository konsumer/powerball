...
describe('numbers', () => {
  ...
  it('should be able to grab past winning numbers in date-range', () => {
    ...
    return lottery.numbers(startDate, endDate)
      .then((num) => {
        assert(Array.isArray(num), 'is an array')
...
describe('frequency', () => {
  ...
  it('should be able to detect frequency of winning numbers in date-range', () => {
    var startDate = new Date('11/25/2015')
    var endDate = new Date('01/02/2016')
    return lottery.frequencies(startDate, endDate)
      .then((freq) => {
        assert(Array.isArray(freq), 'is an array')
        assert.equal(freq.length, 2, 'is white & red balls')
...
describe('statistical analysis', () => {
  ...
  before(() => {
    ...
    var startDate = new Date('11/25/2015')
    var endDate = new Date('01/02/2016')
    return lottery.frequencies(startDate, endDate).then((f) => { freq = f })
  })

  it('should be able to calculate arithmatic mean from a date-range', () => {
    assert.equal(lottery.mean(freq[0]), 1.4286, 'calculate average for white balls')
    assert.equal(lottery.mean(freq[1]), 1.2, 'calculate average for red balls')
  })

  it('should be able to calculate geometric mean from a date-range', () => {
    assert.equal(lottery.gmean(freq[0]), 1.3367, 'calculate average for white balls')
    assert.equal(lottery.gmean(freq[1]), 1.1487, 'calculate average for red balls')
  })

  it('should be able to calculate median from a date-range', () => {
    assert.equal(lottery.median(freq[0]), 1, 'calculate median for white balls')
    assert.equal(lottery.median(freq[1]), 1, 'calculate median for red balls')
  })

  it('should be able to calculate value-range from a date-range', () => {
    var r = lottery.range(freq[0])
  ...
  it('should be able to calculate standard deviation from a date-range', () => {
    assert.equal(lottery.stddev(freq[0]), 0.5408, 'calculate average for white balls')
    assert.equal(lottery.σ(freq[1]), 0.4000, 'calculate average for red balls')
  })
})
